...
class TestLoanRequestCreate:
    ...
    async def test_can_request_item(
        ...
        resp = await bob_client.get("/api/v1/me/borrowings/requests")
        resp.raise_for_status()
        ...
        resp = await alice_client.get(
            f"/api/v1/me/items/{alice_new_item.id}/requests"
        )
        resp.raise_for_status()
        assert request["id"] in {item["id"] for item in resp.json()}
        resp = await alice_client.get(
            f"/api/v1/me/items/{alice_new_item.id}/requests/{request['id']}"
        )
        resp.raise_for_status()
