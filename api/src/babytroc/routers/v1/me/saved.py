...
from babytroc.domains.item.schemas.api import SavedItemApiQuery
...
async def add_item_to_client_saved_items(
    ...
    await item_services.save.add_item_to_user_saved_items(
        db=db,
        user_id=client_id,
        item_id=item_id,
    )
...
async def remove_item_from_client_saved_items(
    ...
    return await item_services.save.remove_item_from_user_saved_items(
        db=db,
        user_id=client_id,
        item_id=item_id,
    )
