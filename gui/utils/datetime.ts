...
export function formatTargetedAge(ageMin: number | null, ageMax: number | null): string {
...
export function ensureDateTime(datetime: DateTime | string | null): DateTime | null;
export function ensureDateTime(datetime: DateTime | string | null): DateTime | null {
...
export function formatRelativeDateRange(dateRange: [DateTime | string | null, DateTime | string | null]): string {
  ...
  if (start !== null) {
    ...
    if (end !== null) {
      ...
      if (start > end)
        throw new Error("Unsorted date range");
      ...
      if (start.hasSame(end, "day")) {
        ...
        if (start.hasSame(now.minus({ days: 1 }), "day"))
          return "Hier";
    ...
    if (end !== null) {
      ...
      if (end.hasSame(now, "day"))
        return "Jusqu'à aujourd'hui";

      if (end.hasSame(now.minus({ days: 1 }), "day"))
        return "Jusqu'a hier";
