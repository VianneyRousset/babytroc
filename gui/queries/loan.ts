...
export function useBorrowingsLoanRequestQuery(loanRequestId: MaybeRefOrGetter<number>) {

  const { $api } = useNuxtApp();

  return useQuery({
    key: () => ["me", "borrowings", "requests", toValue(loanRequestId)],
    query: () => $api("/v1/me/borrowings/requests/{loan_request_id}", {
      path: {
...
export function useItemLoanRequestQuery(
  ...
  return useQuery({
    key: () => ["me", "items", toValue(itemId), "requests", toValue(loanRequestId)],
    query: () => $api("/v1/me/items/{item_id}/requests/{loan_request_id}", {
      path: {
        item_id: toValue(itemId),
        loan_request_id: toValue(loanRequestId),
      }
...
export function useLoanQuery(
  { loanId }: { loanId: MaybeRefOrGetter<number> }
) {
