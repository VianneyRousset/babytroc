...
type FetchResponseData<T extends Record<string | number, any>> = SuccessResponse<ResponseObjectMap<T>, MediaType>
export type FetchResponseError<T extends Record<string | number, any>> = FetchError<ErrorResponse<ResponseObjectMap<T>, MediaType>>
type ParamsOption<T> = T extends { parameters?: any, query?: any } ? T['parameters'] : Record<string, never>
...
type PaginatedSourceOptions<Operation> =
...
>(
  ...
  const $paginatedApi = $api as (url: ReqT, options?: PaginatedSourceOptions<Operation>) => Promise<ResT>;
  ...
  async function more() {
    ...
    if (controller === undefined)
      throw "Uninitialized controller."
    ...
    await $paginatedApi(
      url,
      {
        ...
        async onResponse({ response: { headers } }: { response: FetchResponse<ResT> }) {
