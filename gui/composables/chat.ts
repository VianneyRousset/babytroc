...
export function useChatRoles(chat: MaybeRefOrGetter<Chat>, me: MaybeRefOrGetter<User>) {
  const isUserBorrowing = computed(() => (toValue(chat).borrower.id === toValue(me).id));
  const interlocutor = computed(() => toValue(isUserBorrowing) ? toValue(chat).owner : toValue(chat).borrower);
...
export function useChatMessageOrigin(
  ...
  const origin = computed<ChatMessageOrigin>(() => (
    toValue(message).message_type === ChatMessageType.text
      ? (toValue(message).sender_id === toValue(me).id ? "me" : "interlocutor")
      : "system"));
...
export function useChatMessageTime(message: MaybeRefOrGetter<ChatMessage>) {
  return {
    formattedHour: DateTime.fromISO(toValue(message).creation_date).toLocaleString(DateTime.TIME_SIMPLE),
  }
}
...
export function useGroupChatMessages(
  ...
  return {
    dateGroups: computed(() => {
      ...
      const groups = groupBy(_messages, ({ creation_date }: { creation_date: string }) => DateTime.fromISO(creation_date).toISODate() ?? "") as Record<string, Array<ChatMessage>>;
...
function createMessageDateGroup(date: string, messages: Array<ChatMessage>, user: User): ChatMessageDateGroup {
  return {
    ...
    chunks: groupByMessageChunks(messages.sort((a, b) => b.id - a.id), user),
  }
}

function groupByMessageChunks(messages: Array<ChatMessage>, user: User): Array<ChatMessageChunk> {
