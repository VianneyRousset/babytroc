...
export const useAcceptLoanRequestMutation = defineMutation(() => {
  ...
  return useMutation({
    mutation: ({ itemId, loanRequestId }: { itemId: number, loanRequestId: number }) => $api("/v1/me/items/{item_id}/requests/{loan_request_id}/accept", {
      method: "POST",
      path: {
        item_id: itemId,
    ...
    onError: () => $toast.error("Échec de l'acceptation de la demande d'emprunt."),
    onSettled: (_data, _error, { itemId }) => {
      queryCache.invalidateQueries({ key: ["me", "items", itemId, "requests"] });
    },
  });
});
...
export const useRejectLoanRequestMutation = defineMutation(() => {
  ...
  return useMutation({
    mutation: ({ itemId, loanRequestId }: { itemId: number, loanRequestId: number }) => $api("/v1/me/items/{item_id}/requests/{loan_request_id}/reject", {
      method: "POST",
      path: {
        item_id: itemId,
        loan_request_id: loanRequestId,
...
export const useExecuteLoanRequestMutation = defineMutation(() => {
  ...
  return useMutation({
    mutation: ({ loanRequestId }: { loanRequestId: number }) => $api("/v1/me/borrowings/requests/{loan_request_id}/execute", {
      method: "POST",
      path: {
        loan_request_id: loanRequestId,
      }
...
export const useEndLoanMutation = defineMutation(() => {
  ...
  return useMutation({
    mutation: ({ loanId }: { loanId: number }) => $api("/v1/me/loans/{loan_id}/end", {
      method: "POST",
      path: {
        loan_id: loanId,
      }
