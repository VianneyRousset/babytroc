...
function invalidateItemLists(queryCache: ReturnType<typeof useQueryCache>) {
	queryCache.invalidateQueries(
		{ predicate: (entry) => entry.key.includes("items") },
		"all",
	);
}
...
export const useCreateItemMutation = defineMutation(() => {
	...
	return useMutation({
		mutation: async (context: ItemCreate) =>
			await $api("/v1/me/items", {
				method: "POST",
				body: context,
			}),
...
export function useUpdateItemMutation(itemId: MaybeRefOrGetter<number>) {
	...
	return useMutation({
		...
		mutation: async (context: ItemUpdate) =>
			await $api("/v1/me/items/{item_id}", {
				method: "POST",
				path: {
		...
		onSettled: () => {
			...
			queryCache.invalidateQueries(
				{ predicate: (entry) => entry.key.includes(`item-${toValue(itemId)}`) },
				"all",
			);
		},
	});
