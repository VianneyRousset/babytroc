...
// API query options
type ApiQueryOptions<TPath, TData, Error> = ApiFetchOptions<TPath> &
...
type ParamsOptions<TOperation> = TOperation extends {
	parameters: infer TParams;
}
	? {
			[K in keyof TParams as TParams[K] extends never
				? never
				: K]: MaybeRefOrGetter<TParams[K]>;
		}
	: never;
...
// Helpers
type FetchResponseData<TPath> =
	TPath extends PathWithGetOperationData<infer D> ? D : never;
