...
type ApiPaginatedQueryPaths = {
	[K in keyof paths as paths[K] extends PathWithGetOperationData<Array<unknown>>
		? paths[K] extends PathWithGetOperationQueryParams<never>
			? never
			: K
		: never]: paths[K];
};
...
// Options type
type ApiPaginatedQueryOptions<TPath, TData, TError> = ApiFetchOptions<TPath> &
	Omit<
		UseInfiniteQueryOptions<Array<TData>, TError, Cursor, undefined>,
		"query" | "initialPageParam" | "getNextPageParam"
	>;
...
>(
	url: TReq,
	options: NoInfer<
		ApiPaginatedQueryOptions<ApiPaginatedQueryPaths[TReq], TData, Error>
	>,
): NoInfer<ApiPaginatedQueryReturn<TData, Error>> {
	const queryParams = computed(
		() => pickBy(unref(options.query), (v) => v != null) as TQueryParams,
	);
	const key = computed(() => {
		const _key = toValue(options.key);
		return Array.isArray(_key)
			? [..._key, toRaw(unref(queryParams))]
			: [_key, toRaw(unref(queryParams))];
	});
	...
	async function loadMore() {
		if (unref(error) != null || unref(end) || unref(isLoading)) return;
		await loadNextPage();
	}
...
function extractNextCursorFromHeaders(headers: Headers): Cursor {
	...
	if (!linkHeader?.next) return null;
...
export type FetchResponseDataArray<TPath> =
	TPath extends PathWithGetOperationData<Array<infer D>> ? D : never;
export type FetchQueryParams<TPath> =
	TPath extends PathWithGetOperationQueryParams<infer Q> ? Q : never;
