export const useItemNameValidity = defineValidityFunction<string>(
	(_name) => {
...
export const useItemDescriptionValidity = defineValidityFunction<string>(
	(_description) => {
...
export const useItemRegionsValidity = defineValidityFunction<Set<number>>(
	(_regions) =>
		_regions.size < 1 ? "Veuillez spécifier au moins une région" : undefined,
	{
...
export const useItemImagesValidity = defineValidityFunction<Array<string>>(
	(_images) =>
		_images.length < 1 ? "Veuillez spécifier au moins une image" : undefined,
	{
		initialValue: [],
	},
);
