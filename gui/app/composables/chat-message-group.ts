...
>(
	...
	const dateGroups = computed<Array<ChatMessageDateGroup<MessageT>>>(() => {
		...
		const _dateGroups = Object.entries(
			groupBy(
				_messages,
				({ creation_date }) =>
...
function groupByMessageChunks<
	...
	UserT extends { id: number },
>(messages: Array<MessageT>, me: UserT): Array<ChatMessageChunk<MessageT>> {
	return messages.reduce((acc, msg) => {
		const origin = getChatMessageOrigin(msg, me);

		if (
			acc.length &&
			(acc[acc.length - 1] as ChatMessageChunk<MessageT>).origin ===
				getChatMessageOrigin(msg, me)
		)
			(acc[acc.length - 1] as ChatMessageChunk<MessageT>).messages.push(msg);
		else acc.push(createMessageChunk<MessageT>(origin, [msg]));

		return acc;
	}, [] as ChatMessageChunk<MessageT>[]);
}
