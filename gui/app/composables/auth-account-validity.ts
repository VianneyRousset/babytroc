...
export function useAccountNameValidity(
	...
	return defineValidityFunction<string>(
		(_name) => {
			...
			if (!validCharactersRegex.test(_name) || _error?.status === 400)
...
export const useAccountPasswordValidity = defineValidityFunction<string>(
	(_password) => {
		// validity pattern
		...
		if (
			_password.toLowerCase() === _password ||
			_password.toUpperCase() === _password ||
			!digitPattern.test(_password)
		)
