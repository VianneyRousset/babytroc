...
export function useChatHot<ChatT extends { id: string }>(
	chat: MaybeRefOrGetter<ChatT>,
) {
	const { me } = useMe();

	const liveChatStore = useLiveChatStore();
	const messages: Ref<Array<ChatMessage>> = liveChatStore.getChatMessages(
		() => toValue(chat).id,
	);
...
export function useChats(): {
	...
	const chats = computed<Array<Chat>>(() =>
		[
			...new Map(
				[...unref(queriedChats), ...unref(liveChats)].map((chat) => [
					chat.id,
					chat,
				]),
			).values(),
	...
	const hot = computed(() => {
		const _me = unref(me);
		return (
			_me != null && unref(messages).some((msg) => getChatMessageHot(msg, _me))
		);
	});
