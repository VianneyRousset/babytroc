...
export type ValidityFunction<T> = (
...
export function defineValidityFunction<T>(
	...
	return (value: MaybeRefOrGetter<T>, _touched?: Ref<boolean>) => {
		// throttled touched
		const { value: touched } = useThrottle(
			useTouched<T>(value, options?.initialValue, options?.compare),
			options?.throttle ?? 1000,
		);

		// throttled value
		const { value: throttledValue, synced } = useThrottle(
			value,
			options?.throttle ?? 1000,
		);

		// sync with the given touched value if given
		if (_touched) syncRef(touched, _touched);
		...
		const status = computed<AsyncStatus>(() => {
			...
			if (unref(synced) === false || toValue(options?.pending) === true)
				return "pending";
