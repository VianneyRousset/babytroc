 ...
 **/
export function useChatMessages<ChatT extends { id: string }>(
	...
	const liveMessages: Ref<Array<ChatMessage>> =
		useLiveChatStore().getChatMessages(() => toValue(chat).id);
	...
	const {
		...
		loadMore,
	} = useApiPaginatedQuery("/v1/me/chats/{chat_id}/messages", {
	...
	const messages = computed<Array<ChatMessage>>(() =>
		[
			...new Map(
				[...unref(queriedMessages), ...unref(liveMessages)].map((msg) => [
					msg.id,
					msg,
				]),
			).values(),
...
export function useSendChatMessage<ChatT extends { id: string }>(
	chat: MaybeRefOrGetter<ChatT>,
) {
	...
	const { mutateAsync: send, ...mutation } = useMutation({
		...
		onSettled: () => {
			queryCache.invalidateQueries({
				key: ["chats", toValue(chat).id, "messages"],
			});
		},
	});
...
>(
	...
	const origin = computed<ChatMessageOrigin>(() =>
		getChatMessageOrigin(toValue(message), toValue(me)),
	);
	return { origin };
}
 ...
 **/
export function useChatMessageTime<MessageT extends { creation_date: string }>(
	message: MaybeRefOrGetter<MessageT>,
) {
	return {
