...
export function useItemLoanRequest({
	itemId,
}: {
	...
	request: () => Promise<{
		...
		chatLocation: RouteLocationGeneric;
	}>;
	...
	const error = computed<boolean>(() => {
		return (
			unref(requestMutateStatus) === "error" ||
			unref(unrequestMutateStatus) === "error"
		);
	});
	const loading = computed<boolean>(() => {
		return (
			unref(requestMutateAsyncStatus) === "loading" ||
			unref(unrequestMutateAsyncStatus) === "loading"
		);
	});
