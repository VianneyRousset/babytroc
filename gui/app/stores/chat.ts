...
export const useLiveChatStore = defineStore("live-chat", () => {
	...
	const chatIds = computed<Set<string>>(
		() => new Set([...unref(messages).values()].map((msg) => msg.chat_id)),
	);
	...
	async function ensureChat(chatId: string) {
		...
		unref(savedChats).set(
			chatId,
			await $api("/v1/me/chats/{chat_id}", {
				path: {
					chat_id: chatId,
				},
			}),
	...
	watchEffect(() => {
		for (const chatId of unref(chatIds)) ensureChat(chatId);
	});
