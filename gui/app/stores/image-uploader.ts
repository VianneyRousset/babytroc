...
export const useImageUploader = defineStore("image-uploader", () => {
	...
	function ensure(data: string): AsyncData<string | undefined> {
		...
		const { state, isLoading, error } = useAsyncState<string | undefined>(
			_upload(data),
			undefined,
		);
	...
	function uploadMany(
		data: MaybeRefOrGetter<Array<string>>,
	): AsyncData<Array<string | undefined>> {
		...
		onScopeDispose(
			watchEffect(() => {
				const entries: Array<AsyncData<string | undefined>> =
					toValue(data).map(ensure);
				ret.data.value = entries.map(({ data }) => unref(data));
				ret.status.value = entries.every(
					({ status }) => unref(status) === "success",
				)
					? "success"
	...
	async function _upload(data: string, signal?: AbortSignal): Promise<string> {
